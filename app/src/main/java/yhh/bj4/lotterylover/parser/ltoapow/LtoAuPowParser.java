 ...
 */
public class LtoAuPowParser extends LotteryParser {

    private int mParsePage = 0;
    private Context mContext;

    public LtoAuPowParser(Context context, int parsePage, Callback cb) {
        super(cb);
    ...
    public String getTag() {
        return LtoAuPowParser.class.getSimpleName();
    }

    @Override
    public String getBaseUrl() {
        return "http://www.pilio.idv.tw/ltoAPOW/listbbk.asp";
    }
        ...
        try {
            ...
            for (Element ele : tableTr) {
                ...
                try {
                    ...
                    if (normalNumber.size() != LtoAuPow.getNormalNumbersCount() ||
                            specialNumber.size() != LtoAuPow.getSpecialNumbersCount()) {
                        // TODO failed to get right results
                        continue;
                    }
                    String memo = tds.get(4).text();
                    items.add(new LtoAuPow(seq, drawingTime, normalNumber, specialNumber, memo, ""));
                    if (DEBUG) {
                        Log.v(TAG, "----------");
                    }
                    for (Element td : tds) {
            ...
            if (!items.isEmpty()) {
                ...
                int result = mContext.getContentResolver().bulkInsert(LtoAuPow.DATA_URI, cvs);
                if (result != 0) {
