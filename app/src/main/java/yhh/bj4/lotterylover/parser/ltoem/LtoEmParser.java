 ...
 */
public class LtoEmParser extends LotteryParser {

    private int mParsePage = 0;
    private Context mContext;

    public LtoEmParser(Context context, int parsePage, Callback cb) {
        super(cb);
    ...
    public String getTag() {
        return LtoEmParser.class.getSimpleName();
    }

    @Override
    public String getBaseUrl() {
        return "http://www.pilio.idv.tw/ltoEUROMM/listbbk.asp";
    }
        ...
        try {
            ...
            for (Element ele : tableTr) {
                ...
                try {
                    ...
                    if (normalNumber.size() != LtoEm.getNormalNumbersCount() ||
                            specialNumber.size() != LtoEm.getSpecialNumbersCount()) {
                        // TODO failed to get right results
                        continue;
                    }
                    String memo = tds.get(4).text();
                    items.add(new LtoEm(seq, drawingTime, normalNumber, specialNumber, memo, ""));
                    if (DEBUG) {
                        Log.v(TAG, "----------");
                    }
                    for (Element td : tds) {
            ...
            if (!items.isEmpty()) {
                ...
                int result = mContext.getContentResolver().bulkInsert(LtoEm.DATA_URI, cvs);
                if (result != 0) {
