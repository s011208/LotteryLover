 ...
 */
public class LtoMMParser extends LotteryParser {

    private int mParsePage = 0;
    private Context mContext;

    public LtoMMParser(Context context, int parsePage, Callback cb) {
        super(cb);
    ...
    public String getTag() {
        return LtoMMParser.class.getSimpleName();
    }

    @Override
    public String getBaseUrl() {
        return "http://www.pilio.idv.tw/ltoMM/listbbk.asp";
    }
        ...
        try {
            ...
            for (Element ele : tableTr) {
                ...
                try {
                    ...
                    if (normalNumber.size() != LtoMM.getNormalNumbersCount() ||
                            specialNumber.size() != LtoMM.getSpecialNumbersCount()) {
                        // TODO failed to get right results
                        continue;
                    }
                    String memo = tds.get(4).text();
                    items.add(new LtoMM(seq, drawingTime, normalNumber, specialNumber, memo, ""));
                    if (DEBUG) {
                        Log.v(TAG, "----------");
                    }
                    for (Element td : tds) {
            ...
            if (!items.isEmpty()) {
                ...
                int result = mContext.getContentResolver().bulkInsert(LtoMM.DATA_URI, cvs);
                if (result != 0) {
