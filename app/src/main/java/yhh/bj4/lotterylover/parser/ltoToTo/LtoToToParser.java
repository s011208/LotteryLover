 ...
 */
public class LtoToToParser extends LotteryParser {

    private int mParsePage = 0;
    private Context mContext;

    public LtoToToParser(Context context, int parsePage, Callback cb) {
        super(cb);
    ...
    public String getTag() {
        return LtoToToParser.class.getSimpleName();
    }

    @Override
    public String getBaseUrl() {
        return "http://www.pilio.idv.tw/ltoTOTO/listbbk.asp";
    }
        ...
        try {
            ...
            for (Element ele : tableTr) {
                ...
                try {
                    ...
                    if (normalNumber.size() != LtoToTo.getNormalNumbersCount() ||
                            specialNumber.size() != LtoToTo.getSpecialNumbersCount()) {
                        // TODO failed to get right results
                        continue;
                    }
                    String memo = tds.get(4).text();
                    items.add(new LtoToTo(seq, drawingTime, normalNumber, specialNumber, memo, ""));
                    if (DEBUG) {
                        Log.v(TAG, "----------");
                    }
                    for (Element td : tds) {
            ...
            if (!items.isEmpty()) {
                ...
                int result = mContext.getContentResolver().bulkInsert(LtoToTo.DATA_URI, cvs);
                if (result != 0) {
